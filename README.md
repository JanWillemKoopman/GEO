...
> Zie [abcplan.md](./abcplan.md) voor de volledige technische uitwerking (datamodel, pijplijn-staat-voor-staat, Zod-contracten) en [`supabase/README.md`](./supabase/README.md) voor het complete overzicht van alle migraties.

> **🔍 Praktijktest & voorstel:** de eerste volledige doorloop met een echte klant (Van den Udenhout, "Private Lease Skoda") is nagerekend tegen de opgeslagen data in [praktijktest-udenhout.md](./praktijktest-udenhout.md). Het voorstel dat daaruit volgde — een vragenronde aan de klant vóór het schrijven van content — staat in [contentbriefing.md](./contentbriefing.md). **Let op:** beide documenten zijn geschreven tegen het plan, niet tegen de code; zie de status-kop bovenaan `praktijktest-udenhout.md` voor wat al geverifieerd is.
