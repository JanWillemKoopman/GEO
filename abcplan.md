...
24. **Onboarding-wizard bij profiel-aanmaak, klant-input leidend:** ✅ Een nieuw klantprofiel wordt aangemaakt via een **onboarding-wizard** (meerdere stappen) die naast bedrijfsnaam + website ook een korte omschrijving, branche, producten, concurrenten, tone-of-voice en doelgroep uitvraagt (alles behalve naam + website optioneel). De AI-research (`generateProfileResearch`) vult daarna het profiel, maar **klant-input is leidend**: dit wordt deterministisch in code afgedwongen (`prepare-profile.ts`) — ingevulde scalars (branche, tone-of-voice) blijven staan, producten/concurrenten worden een unie van klant + AI, en lege velden worden door de AI gevuld. Omdat het AI-schema alle velden altijd gevuld teruggeeft, staat er na de merge altijd iets (de klant kan de AI-content aanpassen of overslaan). De twee vrije-tekst seeds (`intake_description`, `intake_audience`) worden bewaard op `profiles` en sturen de AI (en zijn later bewerkbaar). Zie migratie `0006`.
