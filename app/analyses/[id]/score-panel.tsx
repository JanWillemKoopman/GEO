...
/**
 * Score + "jij vs. concurrenten" (abcplan.md §3.5 Overzicht).
 *
 * Herzien in fase 2 (optimalisatie.md 2.3/2.8/2.10). Wat er veranderde en waarom:
 *
  ...
  const change =
    previous && previousLead != null
      ? changeIsMeaningful(
          ...
          {
            score: previousLead,
            stderr: (previous.weighted_score != null ? previous.weighted_stderr : previous.score_stderr) ?? 0,
          },
        )
      : null;
        ...
        <span className="mono-label flex items-center gap-1">
          ...
          <InfoHint label={leadIsWeighted ? "Gewogen zichtbaarheid" : "Zichtbaarheidsscore"}>
            {leadIsWeighted ? (
              ...
              <>
                Van alle vragen die we aan een AI-assistent stelden, in hoeveel word jij genoemd.
                Elke vraag telt even zwaar. Gemeten over {score.judged_runs ?? measuredRunCount} vragen.
              </>
            )}
          </InfoHint>
        </span>
          ...
          {band.margin > 0 && (
            <span className="mb-2 flex items-center gap-1 text-sm text-muted">
              ...
              <InfoHint label="Waarom een marge?">
                ...
                set vragen — of dezelfde vragen op een andere dag — geeft een iets ander getal.
                De echte score ligt naar alle waarschijnlijkheid tussen {band.low} en {band.high}.
              </InfoHint>
            </span>
          )}
        </div>
          ...
          <div className="flex items-center gap-2 border-t border-[var(--border-subtle)] pt-3 text-sm text-secondary">
            <span>
              ...
              {score.score_stderr != null && score.score_stderr > 0 && (
                <span className="text-muted"> ±{confidenceBand(score.score, score.score_stderr).margin}</span>
              )}
            </span>
 ...
 * grafiekje eromheen.
 */
function ChangeLine({ change }: { change: { changed: boolean; delta: number; threshold: number } | null }) {
  ...
  return (
    <p className="flex items-center gap-1 text-sm">
      ...
      <InfoHint label="Echte verandering">
        Dit verschil is groter dan de {change.threshold} punten die we nodig hebben om toeval uit te
