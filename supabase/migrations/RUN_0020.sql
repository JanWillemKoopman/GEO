-- ═══════════════════════════════════════════════════════════════════════════
-- GEO Tracker — migratie van fase 5
--
-- Vervolg op RUN_0012_TOT_0017.sql en RUN_0018_EN_0019.sql. Voer die EERST uit:
-- deze migratie hangt aan `content_pieces` (versies, doelvragen) uit 0019.
--
-- Alles met `if not exists`, dus herhalen is veilig. Er wordt niets verwijderd.
--
-- CONTROLE achteraf:
--   select
--     to_regclass('public.content_impact') is not null as tabel_effect,
--     exists (select 1 from information_schema.columns
--              where table_name = 'content_pieces' and column_name = 'published_at') as publicatie,
--     exists (select 1 from information_schema.columns
--              where table_name = 'tracking_runs' and column_name = 'purpose')        as meting_doel;
-- ═══════════════════════════════════════════════════════════════════════════
...
alter table public.content_pieces
  ...
  add column if not exists publish_check_json jsonb,
  add column if not exists publish_checked_at timestamptz;
...
alter table public.tracking_runs
  ...
  add column if not exists purpose text not null default 'periodic',
  -- Bij welke gepubliceerde pagina deze impact-/controlemeting hoort.
  add column if not exists content_piece_id uuid references public.content_pieces (id) on delete cascade,
  ...
  add column if not exists impact_wave integer;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'tracking_runs_purpose_check') then
    alter table public.tracking_runs
...
create index if not exists tracking_runs_impact_idx
...
create table if not exists public.content_impact (
...
create unique index if not exists content_impact_piece_wave_idx
  on public.content_impact (content_piece_id, wave);
create index if not exists content_impact_analysis_idx on public.content_impact (analysis_id);
...
drop policy if exists "content_impact_select_own" on public.content_impact;
