...
create table public.source_landscape (
...
create unique index source_landscape_analysis_domain_idx
  on public.source_landscape (analysis_id, domain);
create index source_landscape_analysis_idx
...
alter table public.profiles
  ...
  add column entity_checked_at timestamptz;
...
create table public.offsite_tasks (
  id          uuid primary key default gen_random_uuid(),
  analysis_id uuid not null references public.analyses (id) on delete cascade,
...
create unique index offsite_tasks_unique_idx
  on public.offsite_tasks (analysis_id, kind, coalesce(domain, ''));
create index offsite_tasks_analysis_idx on public.offsite_tasks (analysis_id, status);
