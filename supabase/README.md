...
| `0068_functierechten.sql` | Trekt EXECUTE op `is_staff()`, `readable_profile_ids()`, `readable_analysis_ids()` en `user_account_ids()` in bij `public` en `anon`, en geeft hem expliciet aan `authenticated`. ⚠️ Intrekken bij `anon` alléén werkt NIET: PostgreSQL geeft een nieuwe functie standaard EXECUTE aan de pseudo-rol PUBLIC, en `anon` erfde het daarvandaan (zichtbaar als `=X/postgres` in `proacl`). Zet ook een vast `search_path` op `normaliseer_prompt_cluster()` en trekt `usage on schema net` (pg_net, een HTTP-client in de database) in bij `anon` en `authenticated`. ⚠️ Gedragsverandering: een bezoeker zonder sessie kreeg bij een lees-query nul rijen en krijgt nu een weigering. Strenger en dus goed, en nagelopen dat geen enkele publieke pagina met de anon-sleutel leest. Zie `antihack.md` M6 |
