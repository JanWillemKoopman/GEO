...
| `0083_clusterlabels.sql` | Labels op clusters plus de knop naar de prullenbak. Nieuwe tabel `cluster_labels` (per merk, unieke naam op kleine letters) en `analyses.label_id` met `on delete set null`. ⚠️ Een tabel en geen tekstkolom: "Onderhoud", "onderhoud" en "Onderhoud " zouden anders drie groepen in het uitklapmenu zijn waar de gebruiker er één bedoelde. ⚠️ `on delete set null` en geen cascade: een label weggooien mag nooit een cluster meenemen, want het cluster draagt maanden meetdata en het label draagt een woord. ⚠️ De prullenbak voegt GEEN kolom toe: dat is `archived_at` uit `0044`, die er al lag maar nooit een knop had. Daarmee valt een weggehaald cluster via `activeOnly()` per definitie uit de maandelijkse meetronde, dus het meten stopt en er komt geen tweede schakelaar naast. Één selectpolicy op `readable_profile_ids()`, schrijven loopt via de service-role (conventie 6) |
| `0084_sales_procesinzicht.sql` | Een geplande hermeting, de ruwe uitvoer van elke AI-aanroep, en de kosten per markt. Vier kolommen op `sales_markets` (`remeasure_at`, `remeasure_set_by`, `remeasure_done_at`, `remeasure_note`) plus één op `ai_calls` (`raw_json`). ⚠️ `remeasure_done_at` is het slot op de deur: zonder die tweede kolom meet de werker dezelfde markt elke minuut opnieuw zodra de datum voorbij is, en dat is veertig betaalde vragen per keer. ⚠️ `raw_json` is conventie 8 en die ontbrak: van elke aanroep stond wél wat hij kostte, niet wat het model zei. Op 1 september 2026 werd een conceptmail afgekeurd wegens een cijfer dat niet gemeten was, en daarna was niet meer terug te lezen wat er stond. Bewaard wordt de GEPARSTE uitvoer en niet het hele antwoordobject: dat laatste bevat de reasoning-blokken en loopt bij een meting met web-zoeken in de tientallen kilobytes. ⚠️ De index is gedeeltelijk (`where remeasure_at is not null and remeasure_done_at is null`), want er staan hooguit een handvol markten te wachten tussen alle markten |
| `0086_verbeterplan_bestaande_pagina.sql` | De bestaande pagina als bron, en het verbeterplan dat eruit volgt (`docs/tasks/paginakeuze-nieuw-of-verbeteren.md`). Drie kolommen op `content_pieces` (`existing_page_text`, `existing_page_fetched_at`, `related_url`) en één op `planned_pages` (`related_url`). ⚠️ `existing_page_text` is de VERSE tekst van de te verbeteren pagina, tot 6000 tekens, opgehaald op het moment van plannen. Hij vervangt `profile_pages.text_excerpt` als bron voor een verbetering: dat excerpt is afgekapt op 1500 tekens (667 van de 738 gecrawlde pagina's op productie staan op die grens, en 9 van de 10 daadwerkelijk verbeterde pagina's ook) en kan tot 20 dagen oud zijn, terwijl het scherm de klant vertelt zijn pagina ermee te vervangen. ⚠️ Hij wordt BEWAARD en niet alleen gebruikt: het verschilscherm heeft hem nodig, en een herschrijfronde een week later mag niet stilletjes een andere bron krijgen (conventie 8). ⚠️ `related_url` is nadrukkelijk geen tweede `existing_url`: die zegt "deze pagina wordt vervangen", `related_url` zegt "hier staat al iets over dit onderwerp, doe het niet nog eens over". Het rapportmodel wees zo'n pagina 13 keer aan (32 van de 70 nieuw-aanbevelingen droegen een adres, waarvan er 13 echt bestonden) en niets in de keten las hem. ⚠️ Hij staat ook op `planned_pages`, want het normale pad loopt sinds `0065` via de contentvoorraad, en anders verdwijnt het signaal precies op de route die de meeste pagina's afleggen ⚠️ Toegepast op productie onder het label `0083_verbeterplan_bestaande_pagina`: drie takken claimden dat nummer tegelijk, en de andere twee waren eerder op `main`. Het bestand is bij het samenvoegen naar 0086 verschoven, dezelfde soort verschuiving als bij `0081` |
| `0087_inputpoort.sql` | De inputpoort, de onderbouwingsgraad en de sectie achter een vraag (`docs/tasks/vragen-voor-het-schrijven.md`). `content_pieces.input_coverage` en `content_pieces.write_mode`, plus `fact_requests.section_refs`. ⚠️ `input_coverage` meet iets ANDERS dan `coverage_score` uit 0082: dat is de dekking van de geschreven tekst op het contract (achteraf), dit is de dekking van het contract op de feitenkaart (vooraf). Ze horen bij twee verschillende poorten en mogen niet in elkaar geschoven worden. ⚠️ NULL in `input_coverage` is een echte waarde en geen ontbrekende: de pagina heeft geen enkele sectie die iets over dit bedrijf moet zeggen, en een 0 zou de inputpoort daar ten onrechte dichtzetten (conventie 3). ⚠️ `section_refs` is een LIJST van `<content_piece_id>:<sectie-id>` omdat één vraag bij secties van meerdere pagina's kan horen; het sectie-id alleen wijst nergens heen, want elke pagina nummert vanaf s1. ⚠️ `fact_requests.section_id` is in dezelfde migratie aangemaakt en meteen vervangen door `section_refs`; hij is leeg en geen enkele regel code leest hem, maar migraties gooien niets weg (conventie 4) |
...
⚠️ Bij het bouwen van 0099 bleek dat `assignToMonth()` deze vlag nooit op `false` zette, ondanks wat
hierboven al stond: de aanroep ontbrak. Rechtgezet in dezelfde ronde als 0099.

## 0099 — bewust uitgehaald, of nog nooit ingepland (blok A punt 4)

`planned_pages.taken_out`, additief met default `false`. `moveToBacklog()` (`lib/plans.ts`) zet hem
op `true` op het moment dat een klant een kaart terugsleept naar de voorraad; `assignToMonth()` en
`vulOpenMaanden()` zetten hem terug op `false` zodra de kaart weer in een maand terechtkomt. Het
planscherm (`plan-view.tsx`) toont een kaart met `taken_out: true` als "eruit gehaald", en een kaart
die na een volledige vulronde nog steeds in de voorraad zit (`vulOpenMaanden()`'s
