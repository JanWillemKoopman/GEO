...
| `0083_inputpoort.sql` | De inputpoort, de onderbouwingsgraad en de sectie achter een vraag (`docs/tasks/vragen-voor-het-schrijven.md`). `content_pieces.input_coverage` en `content_pieces.write_mode`, plus `fact_requests.section_refs`. ⚠️ `input_coverage` meet iets ANDERS dan `coverage_score` uit 0082: dat is de dekking van de geschreven tekst op het contract (achteraf), dit is de dekking van het contract op de feitenkaart (vooraf). Ze horen bij twee verschillende poorten en mogen niet in elkaar geschoven worden. ⚠️ NULL in `input_coverage` is een echte waarde en geen ontbrekende: de pagina heeft geen enkele sectie die iets over dit bedrijf moet zeggen, en een 0 zou de inputpoort daar ten onrechte dichtzetten (conventie 3). ⚠️ `section_refs` is een LIJST van `<content_piece_id>:<sectie-id>` omdat één vraag bij secties van meerdere pagina's kan horen; het sectie-id alleen wijst nergens heen, want elke pagina nummert vanaf s1. ⚠️ `fact_requests.section_id` is in dezelfde migratie aangemaakt en meteen vervangen door `section_refs`; hij is leeg en geen enkele regel code leest hem, maar migraties gooien niets weg (conventie 4) |
