...
> **Bijgewerkt op 3 september 2026** voor het kwaliteitsraamwerk voor content (migratie `0091`,
> `docs/tasks/contentkwaliteit-framework.md`): §3 (zes kolommen op `content_pieces` plus
> `content_quality_runs` en `content_quality_reviews`), §5 (stap 16, 16a en 16b) en §6 (de vierde
> beoordelaar, en de nieuwe regels in de tabel "bewust géén AI"). De hele weging, de drempels, de
> blokkadeklassen, de zekerheid, de versiekeuze en de root cause zijn rekenkunde en geen AI-aanroep;
> er komt precies één beoordelaar bij, op de goedkope tier, parallel met de bestaande drie.
...
| 16 | Content schrijven | **sol** → vier luna-beoordelaars parallel → sol herschrijven → herbeoordeling | Uitsluitend binnen bevestigde feiten, met per bewering het feit dat hem dekt. Alle keuring staat sinds `0091` op één plek (`lib/pipeline/quality-run.ts`), zodat de eerste versie en de reparatierondes letterlijk dezelfde controles krijgen: vier beoordelaars (redactie, feitelijkheid, citeerbaarheid, vakmanschap) plus tien deterministische controles, daarna wegen tot score, zekerheid en oordeel. `geo_score`, `quality_score` en `coverage_score` blijven onveranderd berekend, zodat die reeksen vergelijkbaar blijven. Schema.org volgt het bedrijfsmodel en draagt een organisatieknoop met `sameAs`. |
| 16a | Kwaliteitsoordeel | | `quality-profile.ts` (per contenttype welke dimensies meetellen en hoe zwaar), `quality-collect.ts` (alle uitkomsten naar getypeerde bevindingen), `quality-score.ts` (score, zekerheid, blokkade als drie losse getallen), `root-cause.ts` (uit welke stap van de keten het probleem komt, puur afgeleid). Geen AI-aanroep: dit weegt alleen wat stap 16 heeft opgeleverd. Zie `docs/tasks/contentkwaliteit-framework.md`. |
| 16b | Kwaliteitslab | | `/beheer/kwaliteit`, staf-only. Het oordeel van de app naast dat van een mens, met de gouden referentie ernaast. Geen AI-aanroep. |
