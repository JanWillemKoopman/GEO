...
| K4 | Het onderzoek schrijft in de kennislaag | 2 | Code gedaan, productie open: `lib/kennis/onderzoek.ts` (omzetting, zelfde indeling en sleutels als K3) en `uit-onderzoek.ts` (via `legVast()`, gooit nooit een fout naar de onderzoeksstap). De vijf stappen schrijven ook nog de oude tabellen. Waargenomen alleen waar de code het citaat terugvond: een sitefeit van de samenvatting, en een aanbodknoop met `confidence = 1` (strenger dan K3). Al het andere afgeleid en intern. Ketenscenario 21 laat de vijf stappen met testtransport draaien en vindt de verwachte items; besluit V18. Nog te doen: na het samenvoegen één nieuw proefmerk op productie aanmaken en de kennislaag ervan nalopen | 26 september 2026 |
