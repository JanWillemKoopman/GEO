...
met zijn functie: een sollicitatieassistent. De achtergrond en de gemaakte keuzes staan in
`docs/logbook.md` (14 september 2026, twee alinea's); dit document gaat alleen over wat er nog open
staat.
...
|---|---|---|
| De assistent | `app/solliciteren/assistent.tsx` en vier onderdelen ernaast | Werkt, nog niet tegen een echte aanroep gedraaid |
| De rekenmodules | `lib/solliciteren/` (modellen, prompt, cliches, sleutelwoorden) | Puur en getest, 107 controles in `test-unit.ts` |
| Het schrijven | `app/api/solliciteren/chats/` | Service-role plus eigenaarscontrole, streamt per regel JSON |
| De opslag | Migratie 0095, `sollicitatie_chats` en `sollicitatie_berichten` | Toegepast op productie en nagerekend |
...
**2. Een model erbij, zodra er een naam vaststaat.** De keuzelijst heeft drie modellen met een
...
**3. Een brief uit het gesprek lichten.** Nu kopieer je het hele antwoord, analyse en al. Een knop
...
**4. Bestanden in plaats van plakken.** Een CV komt meestal uit een PDF. Nu plak je de tekst zelf.
Een PDF uitlezen betekent een extra afhankelijkheid en een opslagplek voor het bestand; het plakken
werkt, dus dit is een gemak en geen gemis.

**5. Een eigen adres, als het ooit een losstaande app wordt.** Hij draait nu op hetzelfde domein en
...
De zes groepen die met "de assistent" beginnen:
